...
// The task is to write a function that, given an area as a positive integer, returns the smallest perimeter possible 
// of a rectangle with integral side lengths.
...
func minimumPerimeter(_ area: Int64) -> Int64 {
  var firstSide = Int64(sqrt(Double(area))) // firstSide < THIS 
  while true {
    if area % firstSide == 0 {
      return 2 * (firstSide + area / firstSide)
    }
    firstSide -= 1
  }
}
