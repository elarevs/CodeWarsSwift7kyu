...
// He bears in mind however, that it’s best to have an extra length of wallpaper handy 
...
func wallpaper(_ l: Double, _  w: Double,_  h: Double) -> String {
    ...
    let akaVolume = 2 * h * (l + w)
    let paper = akaVolume * 1.15 / 5.2
    return paper.truncatingRemainder(dividingBy: 1) == 0 ? numbers[Int((akaVolume * 1.15) / 5.2) ] : numbers[Int(((akaVolume  * 1.15) / 5.2) + 1)]
}
